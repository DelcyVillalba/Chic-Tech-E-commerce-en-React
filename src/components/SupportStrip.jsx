export default function SupportStrip() {
    ...
    return (<section className="bg-white dark:bg-[#0f0c19] transition-colors">
        <div className="max-w-6xl mx-auto px-4 py-10 grid gap-8 sm:grid-cols-2 md:grid-cols-4">
            {items.map((it, i) => (<div key={i} className="group flex items-start gap-3 perspective-900">
                ...
                </div> <div> <h3 className="font-semibold text-[#c2185b] leading-snug">{it.title}</h3>
                    <p className="text-sm opacity-70 dark:text-zinc-200">{it.desc}</p> </div> </div>))}
        </div>
    </section>);
}
